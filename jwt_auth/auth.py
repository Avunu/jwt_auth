...
class JWTAuth:
	...
	def update(self, path: str | None, http_status_code: int | None) -> None:
		self.path = path
		self.http_status_code = http_status_code

	def get_login_url(self, redirect_to: str | None = None) -> str:
...
def handle_redirects(response: Response | None, request: Request) -> None:
	...
	if frappe.session.get("user") == "Guest" and frappe.flags.get("jwt_logout_redirect"):
		...
		response.headers["Location"] = cast(str, frappe.flags.pop("jwt_logout_redirect"))
		return

	if not hasattr(frappe.session, "data") or frappe.session.data is None:
		return

	redirect_to: str | None = frappe.session.data.pop("jwt_auth_redirect", False)
